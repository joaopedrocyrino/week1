...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            rangeProof[i][j + 9] = RangeProof(32);

            rangeProof[i][j].in <== puzzle[i][j];
            rangeProof[i][j].range[0] <== 0;
            rangeProof[i][j].range[1] <== 9;

            rangeProof[i][j].out === 1;

            rangeProof[i][j + 9].in <== solution[i][j];
            rangeProof[i][j + 9].range[0] <== 0;
            rangeProof[i][j + 9].range[1] <== 9;

            rangeProof[i][j + 9].out === 1;
